...
from .export_utils import export_document_translations, get_document_statistics, export_sentences_to_csv
...
class DocumentListView(LoginRequiredMixin, AdminOrRepresentativeMixin, ListView):
    ...
    def get_queryset(self):
        ...
        if sort_by in ['file', '-file', 'uploaded_at', '-uploaded_at', 'uploaded_by__first_name']:
...
class DocumentUploadView(LoginRequiredMixin, AdminOrRepresentativeMixin, View):
    ...
    def post(self, request):
        try:
            ...
            messages.success(request, f'Документ "{document.title}" успешно загружен. Извлечено {len(sentences_data)} предложений.')
...
class DocumentBulkAssignTranslatorView(LoginRequiredMixin, AdminOrRepresentativeMixin, View):
    ...
    def post(self, request, document_id):
        ...
        try:
            ...
            if assigned_count > 0:
                ...
                messages.info(request, 'Все выбранные предложения уже назначены этому переводчику.')
...
class SentenceListView(LoginRequiredMixin, AdminOrRepresentativeMixin, ListView):
    ...
    def get_queryset(self):
        ...
        if sort_by in ['sentence_number', '-sentence_number', 'status', '-status', 'created_at', '-created_at', 'document__file', '-document__file']:
            queryset = queryset.order_by(sort_by)
...
class SentenceDeleteView(LoginRequiredMixin, AdminOrRepresentativeMixin, View):
    ...
    def post(self, request, sentence_id):
        ...
        try:
            ...
            sentence.delete()
            messages.success(request, f'Предложение {sentence_number} из документа "{document_title}" успешно удалено.')
        except Exception as e:
...
class SentenceDetailView(LoginRequiredMixin, DetailView):
    ...
    def get_queryset(self):
        return Sentence.objects.select_related('document', 'assigned_to', 'translation__translator', 'translation__corrector')
    ...
    def post(self, request, sentence_id):
        ...
        if form.is_valid():
            ...
            if sentence.assigned_to:
                ...
                if old_assigned_to:
                    messages.success(request, f'Назначение переводчика снято с предложения №{sentence.sentence_number}')
                else:
...
class TranslationListView(LoginRequiredMixin, AdminOrRepresentativeMixin, ListView):
    ...
    def get_queryset(self):
        ...
        if sort_by in ['translated_at', '-translated_at', 'status', 'translator__first_name']:
            queryset = queryset.order_by(sort_by)
...
def export_sentences(request):
    ...
    allowed_sorts = [
        'document__file', '-document__file', 'sentence_number', '-sentence_number', 'status', '-status', 'created_at', '-created_at'
    ]
