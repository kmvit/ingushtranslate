...
def export_user_report_to_xlsx(user_obj, user_stats, context_data, output_path: str) -> str:
    ...
    try:
        ...
        user_info = [
            ...
            ("Последний вход", user_obj.last_login.strftime("%d.%m.%Y %H:%M") if user_obj.last_login else "Никогда"),
        ]
        ...
        if user_obj.role in ['admin', 'representative']:
            activity_stats = [
                ("Загруженных документов", context_data.get('user_documents', []).count()),
            ]
        ...
        # Заголовки таблицы
        headers = ["Тип", "Слов", "Предложений", "Символов без пробелов", "Символов с пробелами"]
        for col, header in enumerate(headers, 1):
        ...
        if user_stats:
            ...
            stats_sheet.cell(row=row, column=4, value=user_stats.get('total_characters_without_spaces', 0))
            ...
            if user_stats.get('translated_sentences', 0) > 0:
                ...
                stats_sheet.cell(row=row, column=4, value=user_stats.get('total_translated_characters_without_spaces', 0))
                stats_sheet.cell(row=row, column=5, value=user_stats.get('total_translated_characters', 0))
        ...
        if user_obj.role in ['admin', 'representative'] and context_data.get('user_documents'):
            ...
            for row, document in enumerate(context_data['user_documents'][:5], 6):
                ...
                activity_sheet.cell(row=row, column=2, value=document.uploaded_at.strftime("%d.%m.%Y %H:%M"))
        elif user_obj.role == 'translator':
            # Последние переводы
            if context_data.get('user_translations'):
                ...
                for row, translation in enumerate(context_data['user_translations'][:5], 6):
                    ...
                    activity_sheet.cell(row=row, column=2, value=translation.sentence.original_text[:50] + "..." if len(translation.sentence.original_text) > 50 else translation.sentence.original_text)
                    activity_sheet.cell(row=row, column=3, value=translation.translated_text[:50] + "..." if len(translation.translated_text) > 50 else translation.translated_text)
                    status_map = {'approved': 'Утверждено', 'rejected': 'Отклонено', 'pending': 'На проверке'}
                    activity_sheet.cell(row=row, column=4, value=status_map.get(translation.status, translation.status))
                    activity_sheet.cell(row=row, column=5, value=translation.translated_at.strftime("%d.%m.%Y %H:%M"))
        elif user_obj.role == 'corrector':
            # Последние проверки
            if context_data.get('user_corrections'):
                ...
                for row, correction in enumerate(context_data['user_corrections'][:5], 6):
                    ...
                    activity_sheet.cell(row=row, column=2, value=correction.sentence.original_text[:50] + "..." if len(correction.sentence.original_text) > 50 else correction.sentence.original_text)
                    activity_sheet.cell(row=row, column=3, value=correction.translated_text[:50] + "..." if len(correction.translated_text) > 50 else correction.translated_text)
                    status_map = {'approved': 'Утверждено', 'rejected': 'Отклонено', 'pending': 'На проверке'}
                    activity_sheet.cell(row=row, column=4, value=status_map.get(correction.status, correction.status))
                    activity_sheet.cell(row=row, column=5, value=correction.corrected_at.strftime("%d.%m.%Y %H:%M") if correction.corrected_at else correction.translated_at.strftime("%d.%m.%Y %H:%M"))
        ...
        # Заголовки таблицы
        if user_obj.role == 'translator':
            headers = ["Документ", "№ предложения", "Оригинальный текст", "Перевод", "Статус", "Дата перевода"]
        elif user_obj.role == 'corrector':
            headers = ["Документ", "№ предложения", "Оригинальный текст", "Перевод", "Статус", "Дата проверки"]
        else:
            headers = ["Документ", "№ предложения", "Оригинальный текст", "Перевод", "Статус", "Дата"]
        ...
        if user_obj.role == 'translator':
            ...
            for sentence in user_sentences:
                ...
                if hasattr(sentence, 'translation') and sentence.translation:
                    ...
                    status_map = {'approved': 'Утверждено', 'rejected': 'Отклонено', 'pending': 'На проверке'}
                    sentences_sheet.cell(row=row, column=5, value=status_map.get(sentence.translation.status, sentence.translation.status))
                    sentences_sheet.cell(row=row, column=6, value=sentence.translation.translated_at.strftime("%d.%m.%Y %H:%M"))
                else:
            ...
            for correction in user_corrections:
                ...
                status_map = {'approved': 'Утверждено', 'rejected': 'Отклонено', 'pending': 'На проверке'}
                sentences_sheet.cell(row=row, column=5, value=status_map.get(correction.status, correction.status))
                sentences_sheet.cell(row=row, column=6, value=correction.corrected_at.strftime("%d.%m.%Y %H:%M") if correction.corrected_at else correction.translated_at.strftime("%d.%m.%Y %H:%M"))
                row += 1
